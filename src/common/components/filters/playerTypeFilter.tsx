...
export const PlayerTypeFilter = React.memo(({ onChange, selectedOptions }: Props) => {
	...
	const viewPlayerTypeList = React.useMemo(
		() => [
			...
			{
				label: `Free Agents`,
				value: [PlayerTypes.FREE_AGENT, PlayerTypes.TEMPSIGNED],
			},
			{
				label: `Draft Eligible`,
				value: [PlayerTypes.DRAFT_ELIGIBLE],
				isDisabled: isDraftEligibleDisabled,
			},
			{
				label: `Perma FA`,
				value: [PlayerTypes.PERMANENT_FREE_AGENT, PlayerTypes.PERMFA_TEMP_SIGNED],
			},
	...
	return (
		<div className="flex flex-row text-xs m-1">
			...
			<Select
				...
				onChange={onChange}
				defaultValue={
					JSON.parse(localStorage.getItem("viewPlayerTypeOptions") ?? "[]") as MultiValue<{
						label: string;
						value: PlayerTypes[];
					}>
				}
