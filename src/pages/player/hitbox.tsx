...
export function Hitbox ( { hitboxTags }: { hitboxTags: ExtendedStats["hitboxTags"] } ) {
    ...
    const getClassNamesOnPercentage = (percentage: number | string) => {
        ...
        if (+percentage >= 60) {
            ...
            colorClass = '-hue-rotate-[45] saturate-150'; // Gradient from red-400 to red-800 for percentages >= 10%
        } else {
            colorClass = '-hue-rotate-[45] saturate-200'; // Gradient from red-300 to red-700 for percentages < 10%
        }
        return `${colorClass} brightness-150 mix-blend-overlay hover:mix-blend-lighten`;
    }

    console.info(hitboxPercentages);
    return (
        <div className="relative text-sm font-bold h-96 ml-4">
            <img className={`absolute left-[4px] top-[4px]`} src={tpose} alt="tpose" />
            ...
            <img className={`absolute left-[90px] top-[13px] ${getClassNamesOnPercentage(hitboxPercentages.head)}`}     src={head} alt="tpose" />
            <img className={`absolute left-[5px] top-[61px] ${getClassNamesOnPercentage(hitboxPercentages.rightArm)}`} src={rightArm} alt="tpose" />
            <img className={`absolute left-[201px] top-[63px] ${getClassNamesOnPercentage(hitboxPercentages.leftArm)}`}  src={leftArm} alt="tpose" />
            <img className={`absolute left-[134px] top-[64px] ${getClassNamesOnPercentage(hitboxPercentages.chest)}`}    src={chest} alt="tpose" />
            <img className={`absolute left-[127px] top-[138px] ${getClassNamesOnPercentage(hitboxPercentages.stomach)}`}  src={stomach} alt="tpose" />
            <img className={`absolute left-[127px] top-[184px] ${getClassNamesOnPercentage(hitboxPercentages.rightLeg)}`} src={rightLeg} alt="tpose" />
            <img className={`absolute left-[169px] top-[184px] ${getClassNamesOnPercentage(hitboxPercentages.leftLeg)}`}  src={leftLeg} alt="tpose" />
        </div>
    );
}