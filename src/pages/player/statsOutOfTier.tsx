	...
	return (
		<div className={`width-full border rounded-md border-${tier?.color}-600`}>
			<div className="flex justify-center" onClick={() => setIsExpanded(!isExpanded)}>
				...
				<h1 className={`text-xl text-center text-${tier?.color}-600`}>
					{stats.tier}{" "}
					<span className="text-xs text-gray-400">
						<i> Non-primary Tier - {stats.stats.gameCount} games played</i>
					</span>
				</h1>
				<div className="pt-1">
					{isExpanded ?
						<MdKeyboardArrowLeft size="1.5em" className="leading-8 pl-1" />
					:	<MdKeyboardArrowDown size="1.5em" className="leading-8 pl-1" />}
				</div>
			</div>
			...
			>
				<div className="py-2">
					...
					<br />
					{Array(Math.ceil(getGridData(stats.stats).length / 2))
						.fill(0)
						.map((_, i) => {
