...
export const getGridData = (currentStats: CscStats) => {
	...
	return [
		[
			...
			{
				name: PlayerMappings["kast"],
				value: `${(kast * 100).toFixed(2).replace(/[.,]00$/, "")}%`,
				rowIndex: 1,
			},
		],
		[
			{
				name: PlayerMappings["kr"],
				value: `${avgKillsPerRound.toFixed(2)}`,
				rowIndex: 0,
			},
			{
			...
			{
				name: PlayerMappings["multiR"],
				value: `${multiKillRound.toFixed(2)}`,
				rowIndex: 1,
			},
			{
				name: PlayerMappings["clutchR"],
				value: `${(clutchPerRound * 100).toFixed(2)}%`,
				rowIndex: 0,
			},
			{
				name: "2k  /  3k  /  4k  /  Ace",
		...
		[
			{
				name: PlayerMappings["odr"],
				value: `${(openDuelPercentage * 100).toFixed(0)}%`,
				rowIndex: 0,
			},
			// {name: "Entry Frags T-side per Round", value: `${TsidedEntryFragsPerRound}`, rowIndex: 1},
			{
				name: PlayerMappings["tRatio"],
				value: `${(tRatio * 100).toFixed(0)}%`,
				rowIndex: 0,
			},
			{
				name: PlayerMappings["savesR"],
				value: `${savesPerRound.toFixed(2)}`,
				rowIndex: 1,
			},
		],
		[
			{
				name: PlayerMappings["odaR"],
				value: `${openDuelsPerRound.toFixed(2)}`,
				rowIndex: 0,
			},
			// {name: "Average Opening Duel T-side", value: `${avgRoundsOpenDuelOnTside}`, rowIndex: 1},
			{
				name: PlayerMappings["tradesR"],
				value: `${tradesPerRound.toFixed(2)}`,
				rowIndex: 0,
			},
			{
				name: PlayerMappings["awpR"],
				value: `${awpR.toFixed(4)}`,
				rowIndex: 1,
			},
		],
		[
			{
				name: PlayerMappings["util"],
				value: `${utilThrownPerMatch}`,
				rowIndex: 0,
			},
			{
				name: PlayerMappings["utilDmg"],
				value: `${utilDmg.toFixed(2)}`,
				rowIndex: 1,
			},
		...
		[
			...
			{
				name: PlayerMappings["fAssists"],
				value: `${fAssists.toFixed(2)}`,
				rowIndex: 0,
			},
		...
		[
			{
				name: PlayerMappings["suppR"],
				value: `${suppR.toFixed(2)}`,
				rowIndex: 0,
			},
		...
		[
			{
				name: PlayerMappings["suppXR"],
				value: `${suppXR.toFixed(2)}`,
				rowIndex: 0,
			},
