	...
	const playerData = players
		...
		.map(p => {
			...
			return {
				name: p.name,
				tier: p.tier.name,
				team: p.team?.name ?? playerType,
				franchise: p.team?.franchise.prefix ?? "",
				role: p.role,
				mmr: p.mmr,
...
export function ExportData() {
	...
	const filteredByPlayerType =
		viewPlayerTypeOptions?.length ?
			players.filter(player => viewPlayerTypeOptionsCumulative?.some(type => type === player.type))
		:	players;
	...
	const playerData = filteredBySearchPlayers
		.filter(p => Boolean(p.stats))
		.map(p => {
			// Hack to remove properties from stats and include player first class properties
			const { name, team, __typename, rating, ...rest } = p.stats;
			...
			return {
				...
				role: p.role,
				mmr: p.mmr,
				rating: p.stats.rating.toFixed(2),
				...stats,
			};
		});

	const columns: TableColumn<any>[] = Object.keys(playerData[0] ?? {}).map(k => ({
		name: k,
		selector: row => row[k],
		sortable: true,
		reorder: true,
	...
	return (
		<Container>
			<div className="mx-auto max-w-lg text-center">
				...
				<p className="mt-4 text-gray-300">Export a list of players based on filters below as a CSV</p>
					...
					<div className="flex flex-row text-xs my-2 mx-1">
						...
						>
							<span className="flex flex-row">
								<TbDatabaseExport className="mr-2" />
								Export
							</span>
						</button>
					</div>
