...
export function GeneralLeaderBoards( { players, limit} : { players: Player[], limit: number } ) {
    ...
    const twoK = players.sort( (a,b) => (a.stats["twoK"]) < (b.stats["twoK"]) ? 1 : -1).slice(0,limit).map( p => ({ player: p, value: p.stats["twoK"] }));
    const threeK = players.sort( (a,b) => (a.stats["threeK"]) < (b.stats["threeK"]) ? 1 : -1).slice(0,limit).map( p => ({ player: p, value: p.stats["threeK"] }));
    const fourK = players.sort( (a,b) => (a.stats["fourK"]) < (b.stats["fourK"]) ? 1 : -1).slice(0,limit).map( p => ({ player: p, value: p.stats["fourK"] }));
    ...
    const clutchOneVsTwo = _sort(players, "stats.cl_2", limit, "desc").map( p => ({ player: p, value: p.stats['cl_2'].toFixed(2)}));
    const clutchOneVsThree = _sort(players, "stats.cl_3", limit, "desc").map( p => ({ player: p, value: p.stats['cl_3'].toFixed(2)}));
    const clutchOneVsFour = _sort(players, "stats.cl_4", limit, "desc").map( p => ({ player: p, value: p.stats['cl_4'].toFixed(2)}));
