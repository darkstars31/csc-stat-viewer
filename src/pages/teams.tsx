                            ...
                            <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                                { teams.map( team => 
                                        <div className="bg-midnight2 block rounded-xl border border-gray-800 p-6 shadow-xl transition hover:border-pink-500/10 hover:shadow-pink-500/10">
                                            ...
                                            <h4 className="mt-1 text-s font-bold text-gray-500 text-center">({(playerData.filter( p => p.Team === team.Team && p.Tier === tiers[index]).map( p => p.Rating).reduce((cum,cur) => cum + cur, 0)/playerData.filter( p => p.Team === team.Team && p.Tier === tiers[index]).length).toFixed(2)})</h4>
