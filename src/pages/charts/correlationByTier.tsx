...
export function CorrelationByTier({ playerData = [] }: Props) {
	...
	useEffect(() => {
		const newOptions: ChartOption[] = playerStatsKeys
			.map((stat, statIndex) => {
				if (stat !== "rating") {
					const correlationCoefficients: BarData[] = correlationData
						.map(([tier, coefficients]): BarData | undefined => {
