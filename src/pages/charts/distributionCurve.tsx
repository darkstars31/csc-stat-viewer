...
const getByTier = (p: { MMR: number; Tier: string }[], { tier }: { tier: string }) =>
	p.filter(p => p.Tier === tier).flatMap(p => p.MMR);
					...
					<Containers.ChartButtonBox>
						...
						<input
							...
							onChange={e =>
								setExtraFilters({
									...extraFilters,
									mmrGroupingWidth: Number(e.target.value),
								})
							}
						/>
					</Containers.ChartButtonBox>
