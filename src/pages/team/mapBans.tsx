...
export function MapBans({ matches, team }: Props) {
	...
	const tabulateMapBansWithWeights = Object.values(mapBans).reduce(
		(acc, round: { [s: string]: unknown } | ArrayLike<unknown>, index) => {
	...
	const permaBansInOrder = Object.keys(tabulateMapBansWithWeights).sort(
		(a, b) => tabulateMapBansWithWeights[b] - tabulateMapBansWithWeights[a],
	);

	const mapBanMatrixOptions: EChartsOption = {
		title: {
			text: `Ban Matrix`,
			subtext: `PermaBans - ${permaBansInOrder
				.splice(0, 3)
				.map(i => i.split("de_")[1])
				.join(", ")}`,
			subtextStyle: {
