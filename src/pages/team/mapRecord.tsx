	...
	return (
		<div className="p-2">
			<div className="text-center text-sm">
				Total RWP {teamRecord.record.roundsWon}:{teamRecord.record.roundsLost} (
				{calcPercentage(
					teamRecord.record.roundsWon,
					teamRecord.record.roundsWon + teamRecord.record.roundsLost,
				)}
				%)
			</div>
				...
				{Object.values(teamRecord.maps).map(record => (
					<div key={`record ${record.name}`} className="">
						<div className="text-sm text-center">
							<img className="w-16 h-16 mx-auto" src={mapImages[record.name]} alt="FFW" />
						</div>
						<div className="text-center">
							...
							<div className="text-slate-500 text-xs">
								RWP {record.roundsWon}:{record.roundsLost} (
								{calcPercentage(record.roundsWon, record.roundsLost + record.roundsWon)}
								%)
							</div>
						</div>
					</div>
